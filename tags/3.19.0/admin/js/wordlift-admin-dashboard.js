...
jQuery(document).ready(function ($) {
	...
	$('.wl_more_posts').on('click', function (e) {
		...
		wp.ajax.post('wordlift_get_latest_news',
			{more_posts_link_id: $(this).attr('id')}).done(function (data) {
			$.each(data.posts_data, function (index, item) {
				post_url = '<a target = "_blank" href = "' + item.post_url + '">' +
					item.post_title + '</a>';
				// Append each article to news_container
				$('<div>').append(
					post_url,
					$('<p>').html(item.post_description),
