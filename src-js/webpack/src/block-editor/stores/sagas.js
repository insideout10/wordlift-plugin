...
import {
	addEntityRequest,
	addEntitySuccess,
} from '../../classic-editor/components/AddEntity/actions';
...
import {
	relatedPostsRequest,
	relatedPostsSuccess,
} from '../../common/containers/related-posts/actions';
...
function* handleRequestAnalysis() {
	...
	const canCreateEntities =
		'undefined' !== typeof settings['can_create_entities'] &&
		'yes' === settings['can_create_entities'];
...
function embedAnalysis(editorOps, response) {
	...
	if (
		...
		'undefined' === typeof response.annotations
	)
		return;

	const annotations = Object.values(response.annotations).sort(function (
		a1,
		a2
	) {
...
function* toggleEntity({ entity }) {
	...
	const blocks = Blocks.create(
		data.select(EDITOR_STORE).getBlocks(),
		data.dispatch(EDITOR_STORE)
	);
	...
	if (0 === entity.occurrences.length) {
		...
		blocks.replace(
			new RegExp(
				`<span\\s+id="(${annotationSelector})"\\sclass="([^"]*)">`,
				'gi'
			),
			(match, annotationId, classNames) => {
				const newClassNames = mergeArray(
					classNames.split(/\s+/),
					onClassNames
				);
		...
		console.debug(
			`Looking for "<span\\s+id="(${annotationSelector})"\\sclass="([^"]*)"\\sitemid="[^"]*">"...`
		);
		// Switch off.
		blocks.replace(
			new RegExp(
				`<span\\s+id="(${annotationSelector})"\\sclass="([^"]*)"\\sitemid="[^"]*">`,
				'gi'
			),
			(match, annotationId, classNames) => {
				const newClassNames = classNames
					.split(/\s+/)
					.filter((x) => -1 === onClassNames.indexOf(x));
				return `<span id="${annotationId}" class="${newClassNames.join(
					' '
				)}">`;
			}
	...
	data.dispatch(EDITOR_STORE).updateBlockAttributes(
		getClassificationBlock().clientId,
		{
...
function* toggleLink({ entity }) {
	...
	blocks.replace(
		...
		(match, annotationId, classNames) => {
			...
			const newClassNames = classNames
				.split(/\s+/)
				.filter((x) => -1 === cssClasses.indexOf(x));
...
function* toggleAnnotation({ annotation }) {
	...
	blocks.replace(
		new RegExp(
			`<span\\s+id="([^"]+)"\\sclass="(textannotation(?:\\s[^"]*)?)"`,
			'gi'
		),
		(match, annotationId, classNames) => {
...
function* handleAddEntityRequest({ payload }) {
	...
	const annotationId =
		'urn:local-annotation-' + Math.floor(Math.random() * 999999);
...
function* handleSetCurrentEntity({ entity }) {
	...
	}?action=wordlift_redirect&uri=${encodeURIComponent(entity.id)}&to=edit`;
