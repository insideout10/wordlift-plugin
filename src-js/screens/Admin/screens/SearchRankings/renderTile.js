...
function renderTile({ click, select }) {
  return function(node, elem) {
    if (node.data && node.data.entity && node.data.score) {
      ...
      node.data.entity.encodedURI = encodeURIComponent(node.data.entity.itemId);
      ...
      elem.addEventListener("click", () => {
        ...
        const elClassListPrev =
          state && state.node ? state.node.elemClassList : null;
    ...
    return Mustache.render(
      `
        ...
        <div class="tile">
          ...
          }?action=wordlift_redirect&uri={{encodedURI}}&to=edit" class="tile__label">{{label}}</a>
