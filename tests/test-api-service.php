...
class Wordlift_Api_Service_Test extends Wordlift_Unit_Test_Case {
	...
	private function common_assertions( $method, $url, $r ) {
		...
		$this->assertEquals( 1, preg_match( '/^WordLift\/\d+\.\d+\.\d+(\S+)? WordPress\/\d+\.\d+\.\d+ \(multisite:\w+, url:https?:\/\/.+?, locale:\w{2}_\w{2}\) PHP\/\d+\.\d+\.\d+$/', $r['user-agent'] ), "User-Agent must match, got {$r['user-agent']}." );
