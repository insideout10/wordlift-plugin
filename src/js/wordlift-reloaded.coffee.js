...
Traslator = (function() {
  ...
  Traslator.prototype.parse = function() {
    ...
    while ((match = pattern.exec(this._html)) != null) {
      ...
      textPre =
        htmlPre +
        ((ref = htmlElem.toLowerCase()) === "</p>" || ref === "</li>"
          ? "\n\n"
          : "");
      textPost = htmlPost;
  ...
  Traslator.prototype.text2html = function(pos) {
    ...
    textPos = 0;
    for (
      i = j = 0, ref = this._textPositions.length;
      0 <= ref ? j < ref : j > ref;
      i = 0 <= ref ? ++j : --j
    ) {
      ...
      if (
        pos === this._textPositions[i] &&
        this._htmlFragments[i].startsWith("<br")
      ) {
        break;
      }
    }
  ...
  Traslator.prototype.html2text = function(pos) {
    ...
    textPos = 0;
    for (
      i = j = 0, ref = this._htmlPositions.length;
      0 <= ref ? j < ref : j > ref;
      i = 0 <= ref ? ++j : --j
    ) {
  ...
  Traslator.prototype.insertHtml = function(fragment, pos) {
    ...
    this._html =
      this._html.substring(0, htmlPos) +
      fragment +
      this._html.substring(htmlPos);
    return this.parse();
  };
...
angular
  .module("wordlift.utils.directives", [])
  .directive("wlOnError", [
    "$parse",
    "$window",
    "$log",
...
angular
  .module("wordlift.ui.carousel", ["ngTouch"])
        ...
        controller: [
          ...
          function($scope, $element, $attrs, $log) {
            ...
            $scope.isNextArrowVisible = function() {
              return (
                $scope.panes.length - $scope.currentPaneIndex >
                $scope.visibleElements()
              );
            };
            $scope.next = function() {
              if (
                $scope.currentPaneIndex + $scope.visibleElements() + 1 >
                $scope.panes.length
              ) {
                return;
              }
...
angular
  .module("wordlift.editpost.widget.controllers.EditPostWidgetController", [
  ...
  .filter("filterEntitiesByTypesAndRelevance", [
    "configuration",
    "$log",
  ...
  .controller("EditPostWidgetController", [
    "GeoLocationService",
    "RelatedPostDataRetrieverService",
    "EditorService",
    "AnalysisService",
    "configuration",
    "$log",
    ...
    function(
        ...
        switch (entityType) {
          ...
          default:
            ...
            if (!$scope.isThereASelection && $scope.annotation == null) {
              $scope.addMsg(
                "Select a text or an existing annotation in order to create a new entity. Text selections are valid only if they do not overlap other existing annotation",
                "error"
              );
      ...
      $scope.storeCurrentEntity = function() {
        ...
        switch ($scope.currentEntityType) {
          case "entity":
            $scope.analysis.entities[$scope.currentEntity.id] =
              $scope.currentEntity;
      ...
      if (configuration.publishedPlace != null) {
        ...
        $scope.suggestedPlaces[configuration.publishedPlace.id] =
          configuration.publishedPlace;
      }
      ...
      $scope.messages = [];
      RelatedPostDataRetrieverService.load(
        Object.keys($scope.configuration.entities)
      );
      $rootScope.$on("analysisFailed", function(event, errorMsg) {
        $scope.analysisFailed = true;
        return $scope.addMsg(errorMsg, "error");
      });
      $rootScope.$on("analysisServiceStatusUpdated", function(
        event,
        newStatus
      ) {
      ...
      $scope.isLinkedToCurrentAnnotation = function(entity) {
        var ref1;
        return (
          (ref1 = $scope.annotation),
          indexOf.call(entity.occurrences, ref1) >= 0
        );
      };
      $scope.addNewEntityToAnalysis = function() {
        ...
        $scope.analysis.entities[$scope.currentEntity.id].annotations[
          annotation.id
        ] = annotation;
        $scope.analysis.annotations[$scope.annotation].entities[
          $scope.currentEntity.id
        ] = $scope.currentEntity;
        return $scope.onSelectedEntityTile(
          $scope.analysis.entities[$scope.currentEntity.id]
        );
      };
      $scope.$on("updateOccurencesForEntity", function(
        event,
        entityId,
        occurrences
      ) {
      ...
      $scope.$on("textAnnotationAdded", function(event, annotation) {
        $log.debug(
          "[ app.controllers.EditPostWidgetController ] added a new annotation with Id " +
            annotation.id
        );
      ...
      $scope.$on("analysisPerformed", function(event, analysis) {
        ...
        if ($scope.configuration.topic != null) {
          ...
          for (k = 0, len1 = ref1.length; k < len1; k++) {
            ...
            if (
              ((ref2 = topic.id),
              indexOf.call($scope.configuration.topic.sameAs, ref2) >= 0)
            ) {
              $scope.topic = topic;
            }
          }
              ...
              if (entity.occurrences.length === 0) {
                $log.warn(
                  "Entity " +
                    ...
                    " selected as " +
                    box.label +
                    " without valid occurrences!",
                  entity
                );
                continue;
              }
              $scope.selectedEntities[box.id][entityId] =
      ...
      $scope.onSelectedEntityTile = function(entity) {
        ...
        $log.debug(
          "[ app.controllers.EditPostWidgetController ] Action '" +
            action +
            "' on entity " +
            entity.id +
            " within " +
      ...
      $scope.getLocation = function() {
        ...
        $rootScope.$broadcast(
          "geoLocationStatusUpdated",
          $scope.isGeolocationRunning
        );
      ...
      $scope.isPublishedPlace = function(entity) {
        ...
        return (
          entity.id ===
          ((ref1 = $scope.publishedPlace) != null ? ref1.id : void 0)
        );
      };
      ...
      $scope.$on("currentUserLocalityDetected", function(
        event,
        match,
        locality
      ) {
        $log.debug(
          "Looking for entities matching " + match + " for locality " + locality
        );
      ...
      $scope.$on("geoLocationError", function(event, msg) {
        $scope.addMsg(
          "Sorry. Looks like something went wrong and WordLift cannot detect your current position. Make sure the ​location services​ of your browser are turned on.",
          "error"
        );
        $scope.isGeolocationRunning = false;
        return $rootScope.$broadcast(
...
angular
  .module("wordlift.editpost.widget.directives.wlClassificationBox", [])
      ...
      return {
        ...
        templateUrl: function() {
          return (
            ...
            "templates/wordlift-widget-be/wordlift-directive-classification-box.html"
          );
        },
        link: function($scope, $element, $attrs, $ctrl) {
...
angular
  .module("wordlift.editpost.widget.directives.wlEntityList", [])
  .directive("wlEntityList", [
    "$log",
    function($log) {
      return {
        restrict: "A",
...
angular
  .module("wordlift.editpost.widget.directives.wlEntityForm", [])
    ...
    function(configuration, $window, $log) {
      return {
        ...
        templateUrl: function() {
          return (
            configuration.defaultWordLiftPath +
            "templates/wordlift-widget-be/wordlift-directive-entity-form.html?ver=3.24.0-rc1"
          );
        },
        link: function($scope, $element, $attrs, $ctrl) {
          $scope.configuration = configuration;
          ...
          $scope.$watch("entity.id", function(entityId) {
            ...
            if (entityId != null) {
              ...
              category = configuration.getCategoryForType(
                (ref = $scope.entity) != null ? ref.mainType : void 0
              );
          ...
          $scope.isSameAsOf = function(sameAs) {
            var ref;
            return (
              (ref = sameAs.id), indexOf.call($scope.entity.sameAs, ref) >= 0
            );
          };
          $scope.addSameAs = function(sameAs) {
            ...
            if (
              ((ref2 = sameAs.id),
              indexOf.call($scope.entity.sameAs, ref2) >= 0)
            ) {
              index = $scope.entity.sameAs.indexOf(sameAs.id);
              return $scope.entity.sameAs.splice(index, 1);
            } else {
              return (ref3 = $scope.entity) != null
                ? ref3.sameAs.push(sameAs.id)
                : void 0;
            }
          };
          $scope.setType = function(entityType) {
            ...
            return (ref1 = $scope.entity) != null
              ? (ref1.mainType = entityType)
              : void 0;
          };
          ...
          $scope.linkToEdit = function(e) {
            ...
            return ($window.location.href =
              ajaxurl +
              "?action=wordlift_redirect&uri=" +
              $window.encodeURIComponent($scope.entity.id) +
              "&to=edit");
          };
          $scope.hasOccurences = function() {
            var ref;
            return (
              ((ref = $scope.entity.occurrences) != null
                ? ref.length
                : void 0) > 0
            );
          };
          ...
          $scope.isInternal = function() {
            var ref;
            return configuration.isInternal(
              (ref = $scope.entity) != null ? ref.id : void 0
            );
          };
          return ($scope.isNew = function(uri) {
            var ref;
            return !/^(f|ht)tps?:\/\//i.test(
              (ref = $scope.entity) != null ? ref.id : void 0
            );
          });
        }
...
angular
  .module("wordlift.editpost.widget.directives.wlEntityInputBox", [])
    ...
    function(configuration, $log) {
      return {
        ...
        templateUrl: function() {
          return (
            configuration.defaultWordLiftPath +
            "templates/wordlift-widget-be/wordlift-directive-entity-input-box.html?ver=3.24.0-rc1"
          );
        }
      };
    }
...
angular
  .module("wordlift.editpost.widget.services.EditorAdapter", [
    "wordlift.editpost.widget.services.EditorAdapter"
  ])
  .service("EditorAdapter", [
    "$log",
    function($log) {
      var service;
      service = {
        getEditor: function(id) {
          ...
          if (id == null) {
            id =
              (ref = window["wlSettings"]["default_editor_id"]) != null
                ? ref
                : "content";
          }
          return tinyMCE.get(
            (ref1 =
              typeof wp !== "undefined" && wp !== null
                ? (ref2 = wp.hooks) != null
                  ? ref2.applyFilters("wl_default_editor_id", id)
                  : void 0
                : void 0) != null
              ? ref1
              : id
...
angular
  .module("wordlift.editpost.widget.services.AnnotationParser", [])
  .service("AnnotationParser", [
    "$log",
    function($log) {
      var service;
      service = {
...
angular
  .module("wordlift.editpost.widget.services.AnalysisService", [
  ...
  .service("AnalysisService", [
    ...
    function(
        ...
        for (annotationId in ref) {
          ...
          if (annotation.start > 0 && annotation.end > annotation.start) {
            annotationRange = function() {
              results1 = [];
              for (
                var j = (ref1 = annotation.start), ref2 = annotation.end;
            ...
            if (isOverlapping) {
              $log.warn(
                "Annotation with id: " +
                  ...
                  " end: " +
                  annotation.end +
                  " overlaps an existing annotation"
              );
      ...
      service.deleteAnnotation = function(analysis, annotationId) {
        ...
        $log.warn(
          "Going to remove overlapping annotation with id " + annotationId
        );
      ...
      service.parse = function(data) {
        ...
        for (id in ref3) {
          ...
          for (index = l = 0, len2 = ref4.length; l < len2; index = ++l) {
            ...
            data.annotations[id].entities[ea.entityId] =
              data.entities[ea.entityId];
          }
        }
        return data;
      };
      ...
      service.preselect = function(analysis, annotations) {
        var annotation,
          ...
          textAnnotation;
        $log.debug(
          "Selecting " + annotations.length + " entity annotation(s)..."
        );
        results1 = [];
        for (l = 0, len2 = annotations.length; l < len2; l++) {
          ...
          if (annotation.start === annotation.end) {
            $log.warn(
              "There is a broken empty annotation for entityId " +
                annotation.uri
            );
            continue;
          }
          textAnnotation = findAnnotation(
          ...
          if (textAnnotation == null) {
            ...
            textAnnotation = this.createAnnotation({
              ...
              cssClass:
                annotation.cssClass != null ? annotation.cssClass : void 0
            });
          ...
          if (entity == null) {
            $log.warn(
              "Entity with uri " +
                annotation.uri +
                " is missing both in analysis results and in local storage"
            );
            continue;
          }
          analysis.entities[entity.id].occurrences.push(textAnnotation.id);
          if (
            analysis.entities[entity.id].annotations[textAnnotation.id] == null
          ) {
            analysis.entities[entity.id].annotations[
              textAnnotation.id
            ] = textAnnotation;
            ...
            results1.push(
              (analysis.annotations[textAnnotation.id].entities[entity.id] =
                analysis.entities[entity.id])
            );
          } else {
      ...
      service.canCreateEntities =
        wlSettings["can_create_entities"] != null &&
...
angular
  .module("wordlift.editpost.widget.services.EditorService", [
    "wordlift.editpost.widget.services.EditorAdapter",
    "wordlift.editpost.widget.services.AnalysisService"
  ])
  .service("EditorService", [
    ...
    function(
        ...
        for (j = 0, len = entities.length; j < len; j++) {
          ...
          positions = positions.concat(
            function() {
              results1 = [];
              for (
                var k = (ref = entityAnnotation.start),
                  ref1 = entityAnnotation.end;
      ...
      $rootScope.$on("entitySelected", function(event, entity, annotationId) {
        ...
        $log.debug(
          "[ app.services.EditorService ] `entitySelected` event received.",
          event,
          entity,
          annotationId
        );
      ...
      $rootScope.$on("entityDeselected", function(event, entity, annotationId) {
        ...
        console.debug(
          'EditorService::$rootScope.$on "entityDeselected" (event)',
          {
            event: event,
            entity: entity,
      ...
      service = {
        ...
        createTextAnnotationFromCurrentSelection: function() {
          ...
          if (ed.selection.isCollapsed()) {
            $log.warn(
              "Invalid selection! The text annotation cannot be created"
            );
            return;
          }
        ...
        embedAnalysis: (function(_this) {
          return function(analysis) {
            ...
            annotations = Object.values(analysis.annotations).sort(function(
              a,
              b
            ) {
              if (a.end > b.end) {
            ...
            for (j = 0, len = annotations.length; j < len; j++) {
              ...
              if (annotation.entityMatches.length === 0) {
                $log.warn(
                  "Annotation " +
                    ...
                    "] with id " +
                    annotation.id +
                    " has no entity matches!"
                );
                continue;
              }
              element =
                '<span id="' + annotation.id + '" class="textannotation';
              if (
                -1 <
                ((ref = annotation.cssClass) != null
                  ? ref.indexOf("wl-no-link")
                  : void 0)
              ) {
                element += " wl-no-link";
              }
              if (
                -1 <
                ((ref1 = annotation.cssClass) != null
                  ? ref1.indexOf("wl-link")
                  : void 0)
              ) {
                element += " wl-link";
              }
              ...
              for (k = 0, len1 = ref2.length; k < len1; k++) {
                ...
                if (analysis.entities[em.entityId] == null) {
                  $log.warn(
                    em.entityId + " not found in `entities`, skipping."
                  );
                  continue;
                }
                ...
                if (
                  ((ref3 = annotation.id),
                  indexOf.call(entity.occurrences, ref3) >= 0)
                ) {
                  element +=
                    " disambiguated wl-" +
                    entity.mainType +
              ...
              element += '">';
              html =
                html.substring(0, annotation.end) +
                "</span>" +
                html.substring(annotation.end);
              html =
                html.substring(0, annotation.start) +
                element +
                html.substring(annotation.start);
            }
...
angular
  ...
  .service("RelatedPostDataRetrieverService", [
    "configuration",
    "$log",
    "$http",
    "$rootScope",
    function(configuration, $log, $http, $rootScope) {
      var service;
      service = {};
      service.load = function(entityIds) {
        ...
        uri =
          "admin-ajax.php?action=wordlift_related_posts&post_id=" +
          configuration.currentPostId;
        return $http({
          method: "post",
          url: uri,
          data: entityIds
...
angular
  .module("wordlift.editpost.widget.services.GeoLocationService", [
    "geolocation"
  ])
    ...
    function(
      ...
      loadGoogleAPI = function() {
        ...
        callback = function(e) {
          ...
          if (
            element.readyState &&
            ((ref = element.readyState) !== "complete" && ref !== "loaded")
          ) {
            return;
          }
          return $timeout(function() {
...
angular
  .module("wordlift.editpost.widget.providers.ConfigurationProvider", [])
...
(function(_this) {
  return function($, angular) {
    ...
    angular
      .module("wordlift.editpost.widget", [
        "ngAnimate",
    ...
    container = $(
      '<div\n  id="wordlift-edit-post-wrapper"\n  ng-controller="EditPostWidgetController"\n  ng-include="configuration.defaultWordLiftPath + \'templates/wordlift-widget-be/wordlift-editpost-widget.html?ver=3.24.0-rc1\'">\n</div>'
    ...
    injector = angular.bootstrap($("#wordlift-edit-post-wrapper"), [
      "wordlift.editpost.widget"
    ]);
    injector.invoke([
      "$rootScope",
      "$log",
    ...
    if (window["wlSettings"] != null) {
      return tinymce.PluginManager.add("wordlift", function(editor, url) {
        var addClassToBody,
          ...
          startAnalysis;
        defaultEditorId =
          "undefined" !== typeof window["wlSettings"]["default_editor_id"]
            ? window["wlSettings"]["default_editor_id"]
            : "content";
        editorId =
          (ref =
            typeof wp !== "undefined" && wp !== null
              ? (ref1 = wp.hooks) != null
                ? ref1.applyFilters("wl_default_editor_id", defaultEditorId)
                : void 0
              : void 0) != null
            ? ref
            : defaultEditorId;
        console.log(
          "Loading WordLift [ default editor :: " +
            defaultEditorId +
            " ][ target editor :: " +
            editorId +
            " ][ this editor :: " +
        ...
        if (!closed) {
          injector.invoke([
            ...
            "$rootScope",
            "$log",
            function(EditorService, $rootScope, $log) {
              ...
              for (j = 0, len = ref2.length; j < len; j++) {
                ...
                if (wp.mce.views[method] != null) {
                  ...
                  $log.warn(
                    "Override wp.mce.views method " +
                      method +
                      "() to prevent shortcodes rendering"
                  );
                  ...
                  $rootScope.$on("analysisEmbedded", function(event) {
                    $log.info(
                      "Going to restore wp.mce.views method " + method + "()"
                    );
        ...
        startAnalysis = function() {
          return injector.invoke([
            ...
            "$rootScope",
            "$log",
            function(AnalysisService, EditorService, $rootScope, $log) {
...
//# sourceMappingURL=wordlift-reloaded.coffee.js.map
