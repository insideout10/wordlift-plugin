...
test("given list of properties should render the items", () => {
  ...
  expect(component.find(PropertyListItemComponent).find(".wl-property-list-item")).toHaveLength(1);
});

test("given a single property item in edit state should render correctly", () => {
  // 1 edit item should be rendered
  expect(component.find(PropertyComponent).find(".wl-property-edit-item")).toHaveLength(1);
});

test("when close mapping on edit property list item is clicked," + " should convert back to list item", () => {
  /**
   ...
   */
  component
    .find(PropertyComponent)
    .find(".wl-close-mapping")
    .simulate("click");

  // now we should have that item be changed to PropertyListItemComponent
  expect(component.find(PropertyListItemComponent).find(".wl-property-list-item")).toHaveLength(2);
});
...
test("when add mapping is clicked, able to add an property", () => {
   ...
   */
  expect(component.find(PropertyComponent).find(".wl-property-edit-item")).toHaveLength(2);
});

test("when property help text is changed should reflect " + " on PropertyListItemComponent", () => {
  // now write some text on property help text
  component
    .find(PropertyComponent)
    .find(".wl-property-help-text")
  ...
  expect(
    component
      .find(PropertyListItemComponent)
      .at(1)
      .find(".wl-property-list-item-title")
      .props().children
  ).toEqual("something");
});
