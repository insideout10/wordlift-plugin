describe( 'Navigator', () => {
    const postTitle = 'WL QA Test Navigator';
	it( 'Add navigator widget in post', () => {
        ...
        cy.get('.wp-block-paragraph').last().type('/navigator');

        cy.get('#components-autocomplete-item-1-block-wordlift\\/navigator').click();
        ...
        cy.get('.components-button.components-snackbar__action.is-tertiary').then( ( $button ) => {
            ...
            cy.visit( `${href}?amp=1` );
            cy.get('.wl-amp-navigator').should('be.visible');
        });
