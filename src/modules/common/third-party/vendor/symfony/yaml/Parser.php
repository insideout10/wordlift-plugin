...
class Parser {
	...
	private function doParse( $value, $flags ) {
		...
		do {
			...
			} elseif ( self::preg_match( '#^(?P<key>(?:![^\\s]++\\s++)?(?:' . Inline::REGEX_QUOTED_STRING . '|(?:!?!php/const:)?[^ \'"\\[\\{!].*?)) *\\:(\\s++(?P<value>.+))?$#u', \rtrim( $this->currentLine ), $values ) && ( \false === \strpos( $values['key'], ' #' ) || \in_array( $values['key'][0], array( '"', "'" ) ) ) ) {
	...
	private function parseValue( $value, $flags, $context ) {
		...
		try {
			...
			while ( $this->moveToNextLine() ) {
				...
				$escapedLine = \str_replace( array( '\\\\', '\\"' ), '', $this->currentLine );
