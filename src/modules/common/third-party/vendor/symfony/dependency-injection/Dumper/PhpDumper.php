...
use Wordlift\Modules\Common\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\DumperInterface as ProxyDumper;
...
EOF;
	...
	private function analyzeCircularReferences( $sourceId, array $edges, &$checkedNodes, &$currentPath = array(), $byConstructor = \true ) {
	...
	private function connectCircularReferences( $sourceId, &$currentPath, $byConstructor, &$subPath = array() ) {
	...
	private function generateProxyClasses() {
		...
		$strip            = '' === $this->docStar && \method_exists( 'Wordlift\\Modules\\Common\\Symfony\\Component\\HttpKernel\\Kernel', 'stripComments' );
...
EOTXT
	...
	private function startClass( $class, $baseClass, $baseClassWithNamespace ) {
		$bagClass      = $this->container->isCompiled() ? 'use Wordlift\Modules\Common\\Symfony\\Component\\DependencyInjection\\ParameterBag\\FrozenParameterBag;' : 'use Wordlift\Modules\Common\\Symfony\\Component\\DependencyInjection\\ParameterBag\\ParameterBag;';
...
EOF;
	...
	private function getDefinitionsFromArguments( array $arguments, \SplObjectStorage $definitions = null, array &$calls = array(), $byConstructor = null ) {
		if ( null === $definitions ) {
		...
		foreach ( $arguments as $argument ) {
			if ( \is_array( $argument ) ) {
				...
				if ( ! isset( $calls[ $id ] ) ) {
					$calls[ $id ] = array( 0, $argument->getInvalidBehavior(), $byConstructor );
				} else {
				...
				$arguments                = array( $argument->getArguments(), $argument->getFactory() );
				$this->getDefinitionsFromArguments( $arguments, $definitions, $calls, null === $byConstructor || $byConstructor );
				$arguments = array( $argument->getProperties(), $argument->getMethodCalls(), $argument->getConfigurator() );
				$this->getDefinitionsFromArguments( $arguments, $definitions, $calls, null !== $byConstructor && $byConstructor );
			}
		}
		return $definitions;
	...
	private function getExpressionLanguage() {
		if ( null === $this->expressionLanguage ) {
			if ( ! \class_exists( 'Wordlift\\Modules\\Common\\Symfony\\Component\\ExpressionLanguage\\ExpressionLanguage' ) ) {
