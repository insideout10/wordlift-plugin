...
test("when user clicks on add rule group, then rule group is added", () => {
  ...
  expect(MOCK_STORE.getState().RuleGroupData.ruleGroupList).toHaveLength(3);
});
...
test("when user clicks on the and button on first rule group it should add a rule", () => {
  ...
  component
    ...
    .at(0)
    .simulate("click");
  expect(MOCK_STORE.getState().RuleGroupData.ruleGroupList[0].rules).toHaveLength(2);
});

test("when there is no rule in the second rule group" + "after removal then remove that rule group", () => {
  // we removed the only one rule from the second rule group
  component
    .find(".wl-remove-button")
    .at(0)
    .simulate("click");
  expect(MOCK_STORE.getState().RuleGroupData.ruleGroupList).toHaveLength(1);
});
