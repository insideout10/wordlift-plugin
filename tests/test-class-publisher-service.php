...
class Wordlift_Publisher_Service_Test extends Wordlift_Unit_Test_Case {
	...
	public function setUp() {
		...
		$this->publishers_service = new Wordlift_Publisher_Service( $wordlift->get_configuration_service() );
	}
	...
	function test_nopublishers() {
		...
		$this->assertEquals( 0, $this->publishers_service->count() );
		$this->assertEquals( 0, count( $this->publishers_service->query() ) );
		$this->assertEquals( 0, count( $this->publishers_service->query( 'bla' ) ) );
	...
	function test_publishers() {
		...
		$busines = $this->factory->post->create(
			array(
				'post_title'  => 'bla',
				'post_type'   => 'entity',
				'post_status' => 'publish',
		...
		// create a person
		$person = $this->factory->post->create(
			array(
				'post_title'  => 'blabla',
				'post_type'   => 'entity',
				'post_status' => 'publish',
		...
		$this->assertEquals( 1, $this->publishers_service->count() );
		$this->assertEquals( 1, count( $this->publishers_service->query() ) );
		$this->assertEquals( 1, count( $this->publishers_service->query( '' ) ) );

		// create an organization
		$org = $this->factory->post->create(
			array(
				'post_title'  => 'alb',
				'post_type'   => 'entity',
				'post_status' => 'publish',
		...
		$this->assertEquals( 2, $this->publishers_service->count() );
		$this->assertEquals( 2, count( $this->publishers_service->query() ) );
		$this->assertEquals( 2, count( $this->publishers_service->query( '' ) ) );

		// test the search
		$this->assertEquals( 1, count( $this->publishers_service->query( 'bla' ) ) );

		// Test that posts with the relevant entity type are also returned
		// random post article
		$blapost = $this->factory->post->create(
			array(
				'post_title'  => 'blabla',
				'post_type'   => 'post',
				'post_status' => 'publish',
		...
		$postperson = $this->factory->post->create(
			array(
				'post_title'  => 'oblao',
				'post_type'   => 'post',
				'post_status' => 'publish',
		...
		$postorg = $this->factory->post->create(
			array(
				'post_title'  => 'blabla',
				'post_type'   => 'post',
				'post_status' => 'publish',
		...
		$this->assertEquals( 4, $this->publishers_service->count() );
		$this->assertEquals( 4, count( $this->publishers_service->query() ) );
		$this->assertEquals( 4, count( $this->publishers_service->query( '' ) ) );

		// test the search
		$this->assertEquals( 3, count( $this->publishers_service->query( 'bla' ) ) );
	}
