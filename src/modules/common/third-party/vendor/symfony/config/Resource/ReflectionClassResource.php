 ...
 */
namespace Wordlift\Modules\Common\Symfony\Component\Config\Resource;

use Wordlift\Modules\Common\Symfony\Component\DependencyInjection\ServiceSubscriberInterface;
use Wordlift\Modules\Common\Symfony\Component\EventDispatcher\EventSubscriberInterface;
/**
...
class ReflectionClassResource implements SelfCheckingResourceInterface, \Serializable {
	...
	private function generateSignature( \ReflectionClass $class ) {
		...
		if ( \defined( 'Wordlift\\Modules\\Common\\HHVM_VERSION' ) ) {
			...
			foreach ( $class->getMethods( \ReflectionMethod::IS_PUBLIC | \ReflectionMethod::IS_PROTECTED ) as $m ) {
				$defaults                         = array();
				$parametersWithUndefinedConstants = array();
				...
				if ( ! $parametersWithUndefinedConstants ) {
					...
					$stack = array( $m->getDocComment(), $m->getName(), $m->isAbstract(), $m->isFinal(), $m->isStatic(), $m->isPublic(), $m->isPrivate(), $m->isProtected(), $m->returnsReference(), $t instanceof \ReflectionNamedType ? (string) $t->allowsNull() . $t->getName() : (string) $t );
					foreach ( $m->getParameters() as $p ) {
		...
		if ( \interface_exists( EventSubscriberInterface::class, \false ) && $class->isSubclassOf( EventSubscriberInterface::class ) ) {
			...
			( yield \print_r( \call_user_func( array( $class->name, 'getSubscribedEvents' ) ), \true ) );
		}
		if ( \interface_exists( ServiceSubscriberInterface::class, \false ) && $class->isSubclassOf( ServiceSubscriberInterface::class ) ) {
			( yield ServiceSubscriberInterface::class );
			( yield \print_r( \call_user_func( array( $class->name, 'getSubscribedServices' ) ), \true ) );
		}
	}
}
/**
