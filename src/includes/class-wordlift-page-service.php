...
class Wordlift_Page_Service {
	...
	public function handler( $buffer ) {
		...
		// 1. Print out `itemscope itemtype` in the article tag if it's not there
		//    already.
		//
		// 2. Print out the `author` markup (hidden), after the article tag.
		// 3. Print out the `datePublished` markup (hidden), after the article tag.
		// 4. Mark the title as `headline`.
		// 5. Mark the image.
		// 6. Add the `publisher` data.


		// Identify the article block in which microdata needs to be inserted/changed
		// the main article is identified by having a "type-post" in its class which
		// is usually inserted by the post_class function.
