...
function wl_shortcode_faceted_search_ajax( $http_raw_data = NULL ) {
	...
	if ( 'posts' === $required_type ) {
		...
		// see #318: in case of entities, we need to consider the requested entity
		// itself for Ws.
		if (Wordlift_Entity_Service::get_instance()->is_entity($current_post_id))
			$subject_ids += $current_post_id;
